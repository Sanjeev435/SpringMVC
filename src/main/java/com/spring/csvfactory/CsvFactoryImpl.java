...
public class CsvFactoryImpl implements CsvFactory {
	...
	private static final String CSV_FILE = "src\\main\\resources\\userDataBase.csv";
	...
	public User read(User user) throws IOException {
		if (!CollectionUtils.isEmpty(getAllUsers())) {
			for (User userFromService : getAllUsers()) {
				if (userFromService.getFirstname().equalsIgnoreCase(user.getFirstname())
	...
	private void printRecords(CSVPrinter csvPrinter, User user) throws IOException {
		csvPrinter.printRecord(user.getUsername(),
								user.getPassword(),
								user.getFirstname(),
								user.getLastname(),
								user.getEmail(),
